...
public class CharacterAnalyzer {
	...
	public static final boolean isGoodCharacter(char c)
	{
		...
		if(filterUnicode(c))//是unicode符号（标点等），返回false，过滤掉了标点，空格，tab，罗马数字
			return false;
		if(isChinese(c))
			return true;
		if(isWhiteSpace(c))
...
//	    return false;
	...
	public static void main (String[] args){
		//String ss = "、!@#$%^&*北京()！@￥%……&*（）明天！＠＃￥％……＆×";
		String ss ="℃＄¤￠￡‰§№☆★○●◎◇◆□■△▲※→←↑↓〓ⅰⅱ我ⅲⅳⅴⅵⅶ北 京ⅷⅸⅹI        	D123iiivv⒈⒉，你呢？⒊⒋⒌⒍⒎⒏";
		...
		for (int i = 0; i < ch.length; i++) {
			int test = ch[i];
			System.out.println(ch[i]+" - 0x"+Integer.toHexString(test));//输出字符的16进制编码
			if (!isGoodCharacter(ch[i]))//如果不是“好的字符”
			continue;
			str.append(ch[i]);
		}
